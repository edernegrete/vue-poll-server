...
describe('Login example', () => {
  ...
  it('should allow POST requests to the "public/register" route', (done) => {
    request(server)
      ...
      .send({
        username: 'user1', password: 'password1', email: 'user1@gmail.com', name: 'Mr. Lebowski',
      })
  ...
  it('should return 400 for bad POST requests to the "public/register" route', (done) => {
    request(server)
      .post('/public/register')
      .send({
        user: 'user1', password: 'password1', email: 'user1@gmail.com', name: 'Mr. Lebowski',
      })
      .set('Accept', 'application/json')
      .expect(400)
      .end(done);
  ...
  it(
    'should return 406 for trying to register an existing user with the "public/regis' +
      'ter" route',
    (done) => {
      request(server)
        .post('/public/register')
  ...
  it(
    'a POST request to a private route should return "token expired" if the token has' +
      ' expired',
    (done) => {
      request(server)
        .get('/api/v1')
  ...
  it(
    'a POST request to a private route should return "invalid signature" if the token' +
  ...
  it(
    'a POST request to a private route should return "invalid token" if the token is ' +
      'invalid',
    (done) => {
      request(server)
        .get('/api/v1')
  ...
  it(
    'a POST request to a private route should return "jwt malformed" if the token is ' +
      'malformed',
    (done) => {
      request(server)
        .get('/api/v1')
